...
This application is an infrastructure to build a real-time pipeline, targeted for user binded data.
...
This template can be also a base layer for anti-fraud applications. As soon as the transaction comes, it is classified and then it can be used as an input for a rule engine or a neural network for pattern recognition, in order to detect a possible fraud.
